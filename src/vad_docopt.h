...
"   -2 FILE, --alfa2=FLOAT      Marge sobre k1 per determinar el llindar d'un V/S [default: 3.5]\n"
"   -V FILE, --t_voice=FLOAT    Temps mínim per considerar veu [default: 0]\n"
"   -S FILE, --t_silence=FLOAT  Temps mínim per considerar silenci [default: 0.12]\n"
...
int elems_to_args(Elements *elements, DocoptArgs *args, bool help,
    ...
    for (i=0; i < elements->n_options; i++) {
        ...
        } else if (!strcmp(option->olong, "--t_silence")) {
            if (option->argument)
                args->t_silence = option->argument;
